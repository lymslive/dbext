...
" C++/CPP Parser {{{
" @lymslive mondified to support
" printf format, need replace: %c %d %s ... ect
" even in '' string quote
function! s:DB_parseCPP(query)
    ...
    let query = matchstr(query, '"\zs.*\ze"')

    " remove ''\| part from default
    let b:dbext_ignore_variable_regex = '\(in\|out\|inout\|declare\|set\|variable\|/\|@\)'
    let var_expr = '%[cds]'
    " adopt to anonymous named place hoder
    let query = substitute(query, var_expr, '?', 'g')
    let var_expr = '?'
    let query = s:DB_searchReplace(query, var_expr, var_expr, 1)
    call s:DB_set("statement_starts_line", '0')
